...
def main():
    try:
        ...
        while True:
            status, remitdata = InstaRemitRate()
            if status == True:
                rate_instarem = round(remitdata["destination_amount"]/1000, 2)
                instarem_fx_rate = round(remitdata["instarem_fx_rate"], 2)
                fx_rate = round(remitdata["fx_rate"], 2)
                fx_rate_last_updated = remitdata["fx_rate_last_updated"]
            else:
                rate_instarem = ""
            # Remitly
            ...
            rate_all = {"instarem": rate_instarem,
